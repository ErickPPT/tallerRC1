...
char nombreRecursos[MAX_PRODUCTS][MAX_RECURSOS][LONGITUD];
...
void agregarProducto(void) {
    ...
    for (int i = 0; i < variedadRecurso; i++) {
        sprintf(mensaje, "Ingrese la cantidad que se necesita del recurso '%s': ", nombreRecursos[totalProductos][i]);
...
void editarProducto(void) {
    ...
    cantidades[idx] = ingresoDatos("Nueva cantidad demandada: ", 1000, 1);
    ...
    for (int i = 0; i < variedadRecurso; i++) {
        ...
        nombreRecursos[idx][i][strcspn(nombreRecursos[idx][i], "\n")] = '\0';
    }
    for (int i = 0; i < variedadRecursos[idx]; i++) {
        sprintf(mensaje, "Nueva cantidad del recurso '%s': ", nombreRecursos[idx][i]);
        recursos[idx][i] = ingresoDatos(mensaje, 500, 1);
    }
...
void calcularTotales(void) {
    ...
    for (int i = 0; i < totalProductos; i++) {
        ...
        for (int j = 0; j < variedadRecursos[i]; j++) {
            ...
            printf("Producto: %s\n", nombres[i]);
            printf("Cantidad del recurso '%s' requeridos: %ld unidades\n", nombreRecursos[i][j], recursoTotal);
        }
    }
...
void verificarFactibilidad(void) {
    ...
    for (int i = 0; i < totalProductos; i++) {
        for (int j = 0; j < variedadRecursos[i]; j++) {
            sprintf(mensaje, "Ingrese la cantidad disponible del recurso '%s': ", nombreRecursos[i][j]);
            limiteRecursos[i][j] = ingresoDatos(mensaje, 1000, 1);
            recursoTotal = (long)cantidades[i] * recursos[i][j];
            if (recursoTotal > limiteRecursos[i][j]){
                factible = 0; 
                printf("El recurso '%s' no tiene la suficiente cantidad disponible. Necesita %ld mas unidades\n", nombreRecursos[i][j], recursoTotal - limiteRecursos[i][j]);
            }
        }
        if (factible) {
            printf("Factible, se puede cumplir la demanda con respecto a los recursos y el tiempo del producto %d.\n", i+1);
        } else {
            printf("No es factible, no se puede cumplir la demanda con respecto a los recursos y el tiempo del producto %d.\n", i+1);
        }
